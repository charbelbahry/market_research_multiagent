...
async def test_concurrent_analyze():
    ...
    async with AsyncClient(
        ...
        for i in range(5):
            tasks.append(
                client.post(
                    "/analyze", json={"idea": f"Test idea number {i} to prove async"}
                )
            )
...
async def test_timeout_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.core.crew_engine.CrewEngine.analyze", extremely_slow_analyze
    )
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            "/analyze", json={"idea": "Testing timeouts with graceful degradation"}
        )
